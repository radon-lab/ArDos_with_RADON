/*Arduino IDE 1.8.12
  Версия программы RADON v3.0.2 low_pwr 25.09.20 специально для проекта ArDos
...
uint16_t graf_buff[84]; //буфер графика
...
boolean nop_imp = 0; //флаг отсутствия импульсов от счетчика
...
int main(void)  //инициализация
{
  ...
  print("3.0.2", CENTER, 40); //версия по
    ...
    if (!measur) {
      switch (time_wdt) {
        ...
        case TIME_FACT_10: //расчет данных для графика
          ...
          graf_buff[76] = rad_buff[1]; //новое значение в последнюю ячейку
          if (graf_buff[76] > graf_max) graf_max = graf_buff[76];
        ...
        case TIME_FACT_11: //проверка на наличие импульсов от счетчика
          if (!rad_buff[1]) { //если нету импульсов в обенном буфере
            if (nop_imp_tmr++ >= IMP_ERROR_TIME) { //считаем время до вывода предупреждения
              nop_imp = 1; //разрешаем предупреждение
...
void low_pwr(void)
{
  if (sleep_switch && rad_back < RAD_PWR_MANAGER) { //если сон разрешен
...
#else
  EIMSK = 0b00000000; //разрешаем внешнее прерывание INT1
#endif
...
//-------------------------------Тревога----------------------------------------------------------
void alarm_messege(boolean set, boolean sound, char *mode, boolean imp_error) //тревога
{
  ...
  while (1)
  {
    ...
    if (!scr)
    {
      ...
      else print("Ytn cxtnf!", CENTER, 40); //Нет счета!
    }
...
void alarm_warning(void) //обработка тревоги
{
  ...
  if (nop_imp) alarm_switch = 5; //Ошибка счета
  else if (!alarm_back_disable && !alarm_back_wait && rad_back >= alarm_level_back) alarm_switch = 1; //превышение фона 2
  ...
  //-------------------------------------------------------------------------//
  switch (alarm_switch) {
    case 1: alarm_messege(0, alarm_back_sound_disable, "Ajy", 0); break; //фон
    case 2: alarm_messege(1, alarm_dose_sound_disable, "Ljpf", 0); break; //доза
    case 3: warn_messege(0); warn_buzz = 1; break; //фон
    case 4: warn_messege(1); warn_buzz = 1; break; //доза
    case 5: alarm_messege(0, alarm_dose_sound_disable, "", 1); break; //нет импульсов от счетчика
  }
}
//-----------------------------------Первая накачка--------------------------------------------
...
void pump(void) //накачка по обратной связи с АЦП
{
  ...
  for (hv_adc = Read_HV(); hv_adc < ADC_value; hv_adc = Read_HV()) { //значение АЦП при котором на выходе 400В
    ...
    if (++i >= CYCLE_OVERFLOW) {
#if ERRORS_RETURN
      if (hv_adc < 10) error(2); //защита от зацикливания
      else error(1);
#endif
      break;
    }
...
void graf_update(void) //обновление графика
{
  ...
  if (++cnt >= tm) { //расчет показаний в зависимости от фона
    ...
    if (!serch_disable) {
      ...
      graf_buff[76] = rad_scan_buff; //новое значение в последнюю ячейку
      rad_scan_buff = 0; //сбрасываем буфер графика
      if (graf_buff[76] > graf_max) graf_max = graf_buff[76];
  ...
  while (1) {
    ...
    if (!scr) {
      ...
      for (uint8_t i = 0; i < 4; i++) {
        ...
        switch (i) {
          case 0: print("Jgjhyjt&", LEFT, 32); break; //Опорное:
          case 1: print("Bvgekmc&", LEFT, 40); break; //Импульс:
          case 2: printNumF(opornoe, 2, RIGHT, 32, 46, 4, 48); break; //опорное напряжение
          case 3: printNumI(puls, RIGHT, 40); break; //длинна импульса
        }
    ...
    switch (check_keys()) {
      ...
      case 6: //hold select key ///выход в статистику
...
void error(uint8_t err) //ошибки
{
  ...
  if (time_out < time_micros / 100) { //если время вышло, выводим ошибку
    ...
    time_out = time_micros / 100 + ERROR_LENGTHY_TIME; //добавляем n сек. до следущего сообщения
