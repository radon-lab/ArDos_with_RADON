/*Arduino IDE 1.8.12
  Версия программы RADON v3.6.1 fast_mode beta 23.06.21 специально для проекта ArDos
...
*/
//--------------Версия прошивки-------------
#define VERSION_FW 101 //прошивка
#define VERSION_GW 102 //преобразователь
...
#define _RAD_FLASH_OFF {TIMSK0 &= 0b11111110; if (!TIMSK0) PRR |= (1 << 5); RAD_FLASH_OFF;}

volatile uint16_t scan_buff; //переменная для счета импульсов от датчика
volatile uint32_t time_buff; //переменная для счета импульсов от датчика
...
uint32_t time_mid_buff[2]; //массив секундных замеров для усреднения фона
...
int main(void)  //инициализация
{
  ...
  if (eeprom_read_byte((uint8_t*)100) != VERSION_FW) { //если настройки были сброшены, восстанавливаем из переменных
    print(PLEASE, CENTER, 32); //Пожалуйста
    print(WAINT, CENTER, 40); //подождите...

    _delay_ms(START_TIME); //ждем
    setings_update(); //обновляем настройки
    eeprom_update_byte((uint8_t*)100, VERSION_FW); //делаем метку
...
#if DEBUG_RETURN
  if (eeprom_read_byte((uint8_t*)101) != VERSION_GW) { //если настройки преобразователя были сброшены, восстанавливаем из переменных
    pump_update(); //обновляем настройки преобразователя
    eeprom_update_byte((uint8_t*)101, VERSION_GW); //делаем метку
  }
  ...
  TIMSK2 = 0b00000001; //разрешаем прерывания Таймера2
...
void initTimers(void) //инициализация таймеров
{
  ...
  OCR0A = 0b00000000; //устанавливаем OCR0A в начало
  ...
  TCCR2B = 0b00000111; //пределитель 1024
...
void _LIGHT_ON(void) { //подсветка старт
  ...
  TIMSK0 |= 0b00000110; //разрешаем прерывания
...
void _LIGHT_STOP(void) { //подсветка стоп
  ...
  TIMSK2 &= 0b11111001; //отключаем прерывания
  if (!TIMSK0) PRR |= (1 << 6); //выключаем питание таймера
}
//-------------------------------Включение WDT----------------------------------------------------
...
void data_convert(void) //преобразование данных
{
  ...
  static uint32_t tmp_low_time; //общий буфер
  static uint32_t tmp_high_time; //общий буфер
    ...
    if (!measur && !serch) {
      ...
      switch (time_wdt) {
        ...
        case TIME_FACT_5: //расчет текущего фона этап-3
          if (rad_buff[0]) time_mid_buff[0] /= rad_buff[0];
          tmp_high_time = time_mid_buff[1] + (time_mid_buff[1] / 100.0 * _init_accur(rad_buff[1]));
          break;

        case TIME_FACT_6: { //расчет текущего фона этап-4
            uint32_t temp = time_mid_buff[1] / 100.0 * _init_accur(rad_buff[1]);
            if (time_mid_buff[1] > temp) tmp_low_time = time_mid_buff[1] - temp;
            else tmp_low_time = 0;
            time_mid_buff[1] = time_mid_buff[0];
          }
          break;

        case TIME_FACT_7: //расчет текущего фона этап-5
          if (tmp_low_time > time_mid_buff[0] || time_mid_buff[0] > tmp_high_time) { //если видим скачок или спад
            tmp_buff = 0; //сбрасываем текущий буфер
            for (uint8_t i = 0; i < 2; i++) tmp_buff += rad_buff[i]; //запоняем буффер первого плеча
        ...
        case TIME_FACT_8: { //расчет текущего фона этап-6
            ...
            for (uint8_t i = 0; i < PATTERNS_APROX; i++) { //выбор паттерна
              if (imp_per_sec <= pgm_read_word(&back_aprox[i][0])) { //если имп/с совпадают с паттерном
                rad_back = imp_per_sec * (geiger_time + pgm_read_word(&back_aprox[i][1])) - pgm_read_word(&back_aprox[i][2]) * 10.0; //рассчитываем фон в мкр/ч
                break;
              }
            }
#else
...
#endif
            if (geiger_time_now > 1) rad_back = tmp_buff * (geiger_time / ((uint16_t)mid_time_now * BUFF_LENGTHY + back_time_now)); //расчет фона мкР/ч
#endif
...
{
  if (sleep_switch == 2 && power_manager) { //если сон разрешен и разрешено энергосбережение
    if (TIMSK1 || TIMSK0) waint_pwr(); //если включен бузер или шим подсветки или индикация частиц - режим ожидания
...
ISR(TIMER0_COMPA_vect) //плавная подсветка
{
  ...
  switch (light_switch) { //выбераем режим 1 - разгорание | 0 - затухание
    case 0:
      ...
      break;
    case 1:
      if (OCR0B < 255) OCR0B++; //прибавляем счетчик циклов шим
      else {
        LIGHT_ON; //включаем подсветку
        _LIGHT_STOP(); //выключаем все
      }
...
//---------------------------------Переполнение счета времени---------------------------------------
ISR(TIMER2_OVF_vect) //переполнение счета времени
{
  time_buff += 255;
}
...
#else
          _logbook_data_update(2, pgm_read_byte(&diff_measuring[measur_pos]), (first_froze < second_froze) ? (second_froze - first_froze) * (geiger_time / (pgm_read_byte(&diff_measuring[measur_pos]) * 60)) : 0); //обновление журнала мкР/ч
#endif
        }
#else
#if  TYPE_MEASUR_LOGBOOK
          ...
          if (next_measur) {
            switch (n) {
              ...
              case 1:
                ...
                _init_rads_unit(1, buff * (geiger_time / (pgm_read_byte(&diff_measuring[measur_pos]) * 60)), 1, 4, 1, 8, 0, 54, 16); //результат мкр/ч
                n = 0;
                break;
            }
          }
...
void start_pump(void) //первая накачка
{
  ...
  for (hv_adc = Read_HV(); hv_adc < ADC_value; hv_adc = Read_HV()) { //значение АЦП при котором на выходе 400В
    ...
    _screen_line(0, map(hv_adc, 0, ADC_value, 0, 64), 0, 10, 40); //прогресс бар накачки преобразователя
  }
}
