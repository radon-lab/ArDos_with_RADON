...
#define MESSUR_OR_PWR     0 //выбор пункта быстрого меню(или выключение питания или разностный замер)(требуется optiboot v8) (1 - выключение питания | 0 - разностный замер)
...
#define SEARCH_RETURN     1 //вернуть режим поиска(требуется optiboot v8) (1 - поиск доступен из интерфеса | 0 - поиск не доступен из интерфейса, график отображается в режиме "ФОН")
...
#define PARAM_RETURN     0 //вернуть параметры устройства(требуется optiboot v8) (1 - параметры доступны из интерфеса | 0 - параметры не доступны из интерфейса)
#define ERRORS_RETURN    0 //вернуть ошибки преобразователя(требуется optiboot v8) (1 - ошибки разрешены | 0 - ошибки запрещены)
#define PWR_ON_RETURN    1 //вернуть включение питания по удержанию клавиши(требуется optiboot v8) (1 - включение по удержанию разрешено | 0 - включение по удержанию запрещено)
