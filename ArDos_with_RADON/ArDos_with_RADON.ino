    ...
    if (mainTask != POWER_DOWN_PROGRAM) { //если не инициализация
      ...
      switch (tick_switch) { //основной блок обрабоки данных
        ...
        case TASK_UPDATE_SCREEN: //таймер обновления экрана
          if (!power_sleep && (mainTask != SEARCH_PROGRAM)) screen_update = 0; //устанавливаем флаг для обновления экрана
          break;
      }
    }
...
#if ALARM_AUTO_DISABLE
      if (buttonState() || (!set && !alarm_switch)) //если нажата любая кнопка или фон упал то отключаем тревогу
#else
      if (buttonState()) //если нажата любая кнопка то отключаем тревогу
#endif
      {
        _vibro_off(); //выключаем вибрацию
        ...
        alarm_switch = 0; //устанавливаем признак отсутствия тревоги
        screen_update = 0; //разрешаем обновление экрана
        return;
      }
