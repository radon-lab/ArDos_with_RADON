...
void _read_memory(void) //проверка и чтение данных из памяти
{
  ...
  for (uint8_t n = 0; n < sizeof(debugSettings); n++) _check_crc(&crc, *((uint8_t*)(&debugSettings) + n)); //рассчитываем контрольную сумму для настроек преобразователя
...
#endif
#if DEBUG_RETURN
    if (checkData(sizeof(debugSettings), EEPROM_BLOCK_SETTINGS_DEBUG, EEPROM_BLOCK_CRC_DEBUG)) dataReg |= 0x04; //проверяем контрольную сумму настроек преобразователя
#endif
  ...
  for (uint8_t i = 0; i < 5; i++) {
    if (dataReg & (0x01 << i)) { //если установлен флаг очистки
...
#endif
#if DEBUG_RETURN
        case 2: updateData((uint8_t*)&debugSettings, sizeof(debugSettings), EEPROM_BLOCK_SETTINGS_DEBUG, EEPROM_BLOCK_CRC_DEBUG); break; //сбрасываем настройки преобразователя
#endif
      ...
      switch (i) { //иначе загружаем данные
        ...
        case 2: EEPROM_ReadBlock((uint16_t)&debugSettings, EEPROM_BLOCK_SETTINGS_DEBUG, sizeof(debugSettings)); break; //загружаем настройки преобразователя
#endif
...
boolean _system_task(void) //основная задача
{
  ...
  if (tick_buff) { //если был тик то обрабатываем данные
    ...
    if (mainTask != POWER_DOWN_PROGRAM) { //если не инициализация
      ...
      time_total += debugSettings.wdt_period; //добавляем ко времени период таймера
      ...
      if (mainTask != MEASUR_PROGRAM && mainTask != SEARCH_PROGRAM) { //если не идет замер/поиск
        switch (tick_switch) { //блок обработки данных режима фон/доза
...
#endif
              if (geiger_time_now > 1) rad_back = temp_buff * (debugSettings.geiger_time / ((uint16_t)mid_time_now * 60 + back_time_now)); //расчет фона мкР/ч
#endif
            }
            break;
          ...
          case TASK_CALC_DOSE: { //расчет текущей дозы
              ...
              uint16_t puls_per_ur = (3600 / debugSettings.geiger_time) + (rad_sum_timer * OWN_BACK);
...
#else
              uint16_t puls_per_ur = 3600 / debugSettings.geiger_time;
              if ((rad_sum += rad_buff[0]) >= puls_per_ur) {
                rad_dose += rad_sum / puls_per_ur;
                rad_sum = rad_sum % puls_per_ur;
              }
      ...
      switch (tick_switch) { //основной блок обрабоки данных
...
#if !PUMP_FEEDBACK
            if (hv_adc < (debugSettings.adc_value - HV_ADC_MIN)) { //если значение АЦП преобразователя ниже на установленное значение
#if LOGBOOK_RETURN
        ...
        case TASK_UPDATE_POWER: { //управление энергосбережением
            if (!sleep_disable) { //если сон разрешен
              uint16_t _imp_per_second = rad_back / debugSettings.geiger_time; //получили имп/с
...
#if DEBUG_RETURN
    case 1:
      if (!(checkDataMemory((uint8_t*)&debugSettings, sizeof(debugSettings), EEPROM_BLOCK_CRC_DEBUG))) return;
      break;
#endif
#if LOGBOOK_RETURN
    case 2:
...
#if DEBUG_RETURN || PUMP_READ_MEM
      case 1: EEPROM_ReadBlock((uint16_t)&debugSettings, EEPROM_BLOCK_SETTINGS_DEBUG, sizeof(debugSettings)); break; //считываем настройки из памяти
#endif
#if LOGBOOK_RETURN
...
#if DEBUG_RETURN
      case 1: updateData((uint8_t*)&debugSettings, sizeof(debugSettings), EEPROM_BLOCK_SETTINGS_DEBUG, EEPROM_BLOCK_CRC_DEBUG); _init_battery(); break; //обновляем настройки
#endif
#if LOGBOOK_RETURN
...
#else
          measur_back = (measur_imp) ? (debugSettings.geiger_time * (_measur_get_imp_per_min(measur_imp) / 60)) : 0; //текущий результат замера в мкр/ч
#endif
...
uint8_t searchMenu(void) //инициализация режима поиск
{
  ...
  search_time_now = (float)pgm_read_word(&search_time[mainSettings.search_pos]) / ((float)debugSettings.wdt_period / 100.0); //переключатель динамического изменения времени
  ...
  while (1) {
    if (_system_task()) { //обработка данных
      ...
      if (_check_screen()) {
        ...
        printNumF(debugSettings.reference, 2, RIGHT, 40, 46, 4, 48); //опорное напряжение
#else
        print(P_PUMP_SPEED, LEFT, 24); //Скорость:
        printNumI(speed_hv, RIGHT, 24);//скорость накачки

        print(P_REFERENCE, LEFT, 32); //Опорное:
        printNumF(debugSettings.reference, 2, RIGHT, 32, 46, 4, 48); //опорное напряжение
#endif
      }
    }
  }
  ...
  while (1) {
    if (_system_task()) { //обработка данных
      switch (_button_state()) {
        ...
        case UP_KEY_KEEP: //удержание кнопки вверх
          switch (cursor) {
            case 0: if (debugSettings.wdt_period < MAX_WDT_PERIOD) debugSettings.wdt_period++; break; //период
            case 1: if (debugSettings.geiger_time < MAX_GEIGER_TIME) debugSettings.geiger_time += (debugSettings.geiger_time < 100) ? 0.1 : 1; break; //счет
            case 2: if (debugSettings.reference < 1.50) debugSettings.reference += 0.01; break; //прибавляем опорное напряжение
            case 3: if (debugSettings.induction < 700) debugSettings.induction += 10; break; //прибавляем длинну импульса
#if !PUMP_FEEDBACK
            case 4: if (debugSettings.k_delitel < 1500) debugSettings.k_delitel++; break; //прибавляем коэффициент делителя
            case 5: if (debugSettings.adc_value < 254) debugSettings.adc_value++; break; //прибавляем значение АЦП для преобразователя
#endif
          }
          break;
        ...
        case DOWN_KEY_KEEP: //удержание кнопки вниз
          switch (cursor) {
            case 0: if (debugSettings.wdt_period > MIN_WDT_PERIOD) debugSettings.wdt_period--; break; //период
            case 1: if (debugSettings.geiger_time > MIN_GEIGER_TIME) debugSettings.geiger_time -= (debugSettings.geiger_time < 100) ? 0.1 : 1; break; //счет
            case 2: if (debugSettings.reference > 0.50) debugSettings.reference -= 0.01; break; //убавляем опорное напряжение
            case 3: if (debugSettings.induction > 40) debugSettings.induction -= 10; break; //убавляем длинну импульса
#if !PUMP_FEEDBACK
            case 4: if (debugSettings.k_delitel > 10) debugSettings.k_delitel--; break; //убавляем коэффициент делителя
            case 5: if (debugSettings.adc_value > 10) debugSettings.adc_value--; break; //убавляем значение АЦП для преобразователя
#endif
          }
          break;
      ...
      if (_check_screen()) {
        ...
        printNumI(debugSettings.wdt_period, 20, 24); //период
        printNumF(debugSettings.geiger_time, (debugSettings.geiger_time < 100), RIGHT, 24, 46, 4, 32); //счёт
        printNumF(debugSettings.reference, 2, 20, 32, 46, 4, 48); //опорное напряжение
        printNumI(debugSettings.induction, RIGHT, 32); //длинна импульса
#if !PUMP_FEEDBACK
        printNumI(debugSettings.k_delitel, 20, 40, 4); //коэффициент делителя
        printNumI(debugSettings.adc_value, RIGHT, 40); //значение АЦП для преобразователя
#endif
