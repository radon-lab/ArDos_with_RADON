/*Arduino IDE 1.8.12
  Версия программы RADON v3.1.8 low_pwr 28.10.20 специально для проекта ArDos
...
int main(void)  //инициализация
{
  ...
  print("3.1.8", CENTER, 40); //версия по
...
void initTimers(void) //инициализация таймеров
{
  ...
  TIMSK0 = 0b00000001; //прерывание по переполнению Таймера0
...
void data_convert(void) //преобразование данных
{
  ...
  static uint32_t beta_buff; //бета буфер
      ...
      switch (time_wdt) {
        ...
        case TIME_FACT_4: //расчет текущего фона этап-2
          if (++tmr_mid >= BUFF_LENGTHY) { //если основной буфер перезаписался
            for (uint8_t k = MID_BUFF_LENGTHY; k > 0; k--) rad_mid_buff[k] = rad_mid_buff[k - 1]; //перезапись массива
...
void low_pwr(void)
{
  if (sleep_switch == 2 && power_manager) { //если сон разрешен и и разрешено энергосбережение
    if (TIMSK1 || TIMSK2 || alarm_switch) waint_pwr(); //если включен бузер или шим подсветки или тревога - режим ожидания
  ...
  if (!scr) { //обновление дисплея
    ...
    switch (scr_mode) { //отрисовываем выбранный экран
      case 0: //режим измерения текущего фона
        switch (alarm_switch) {
          case 0:
            _screen_line(map(constrain(geiger_time_now, 0, BUFF_LENGTHY), 0, BUFF_LENGTHY, 5, 82), map(constrain(mid_time_now, 0, MID_BUFF_LENGTHY), 0, MID_BUFF_LENGTHY, 5, 82), 1, 1, 24); //шкалы точности и усреднения
            break;
