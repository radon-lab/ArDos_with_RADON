...
int main(void)  //инициализация
{
  ...
  PRR = 0b11101110; //отключаем все лишнее (I2C | TIMER2 | TIMER1 | SPI | UART)
  ...
  for (timer_millis = FONT_TIME; timer_millis && !check_keys();) { //ждём
...
{
  TCCR0A = 0b00000000; //отключаем OC1A/OC1B
...
ISR(TIMER1_COMPA_vect) //прерывание сигнала для пищалки
{
  ...
  switch (--cnt_puls) { //считаем циклы времени работы бузера
    case 0: BUZZ_CLR; SOUND_STOP; break; //если циклы кончились, выключаем таймер
  }
}
...
void wdt_calibrate(void) //калибровка wdt
{
  ...
  TIMSK0 = 0b00000000; //отключаем прерывания Таймера2 //отключаем OC1A/OC1B
  TCCR0A = 0b00000000; //отключаем OC1A/OC1B
  TCCR0B = 0b00000001; //пределитель 1

  _delay_ms(START_TIME); //ждем

  uint8_t timeout = 0; //тайм-аут калибровки таймера
  ...
  while (wdt_period < 1600 || wdt_period > 1850) {
    ...
    while (!(WDTCSR & (1 << WDIF))) { //ждем таймаута
      if (TIFR0 & (1 << TOV0)) { //ждем флага 10 мкс
        ...
        TCNT0 = 96; //устанавливаем счетчик таймера в начальное положение
        TIFR0 |= (1 << TOV0); //сбрасываем флаг прерывания по переполнению
      }
    }
    wdt_period = time_micros + TIME_CORRECT; //вычисляем время с коррекцией
    WDTCSR |= (1 << WDCE) | (1 << WDE); //разрешаем вмешательство
          ...
          case 1:
            switch (dose_mode) {
              case 0: //текущая доза
                ...
                for (timer_millis = MASSEGE_TIME; timer_millis && !check_keys();) {
                  low_pwr(); //отключение дисплея и подсветки, уход в сон для экономии энергии
                  pump(); //накачка по обратной связи с АЦП
                  data_convert(); //преобразование данных
                }
