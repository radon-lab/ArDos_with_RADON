/*Arduino IDE 1.8.13
  Версия программы RADON v4.3.4 low_pwr release 21.07.23 специально для проекта ArDos
...
#define SOUND_STOP        TIMSK1 = 0x00; PRR |= (0x01 << PRTIM1); sleep_status &= ~(0x01 << WAIT_TIM1)
...
#define TASK_CALC_BACK_6   15 //расчет текущего фона этап-6
#define TASK_CALC_BACK_7   17 //расчет текущего фона этап-7
#define TASK_CALC_BACK_8   19 //расчет текущего фона этап-8
...
boolean low_bat_massege = 1; //флаг разрешения вывода сообщения об разряженой акб
...
boolean power_sleep = 0; //флаг активного сна
boolean power_light = 0; //флаг выключенной подсветки
...
void _wdt_enable(void) //включение WDT
{
  ...
  WDTCSR = (0x01 << WDIE); //устанавливаем пределитель 2(режим прерываний)
  RESET_WDT; //сброс WDT
...
void _wdt_disable(void) //выключение WDT
{
  ...
  sleep_status &= ~(0x01 << WAIT_WDT); //сбросили флаг запрета сна
}
//---------------------------------Прерывание ADC----------------------------------------------------
...
boolean _data_update(void) //преобразование данных
{
  ...
  if (main_buff) { //если есть импульс в основном буфере
    ...
    if (!timer_flash) { //если индикация не включена
      switch (mainSettings.rad_flash) { //в зависимости от режима
        ...
        case 2: if (!power_sleep) _RAD_FLASH_ON(); break; //индикация попадания частиц
      }
    }
  ...
  if (tick_buff) { //если был тик то обрабатываем данные
    if (--tick_buff) sleep_status |= (0x01 << WAIT_WDT); //убавили тик
    else sleep_status &= ~(0x01 << WAIT_WDT); //разрешили сон
    ...
    if (mainTask != POWER_DOWN_PROGRAM) { //если не инициализация
        ...
        switch (tick_switch) { //блок обработки данных режима фон/доза
          ...
          case TASK_CALC_BACK_6: //расчет текущего фона этап-6
            if (geiger_time_now >= GEIGER_CYCLE) { //если массив заполнен на минимум начала работы коэффициентов
              ...
              temp_right = 0; //сбрасываем буффер второго плеча
          ...
          case TASK_CALC_BACK_7:  //расчет текущего фона этап-7
            if (geiger_time_now >= GEIGER_CYCLE) { //если массив заполнен на минимум начала работы коэффициентов
              ...
              debug_coef = coef_main; //установленное соотношение ячеек сравнения
#endif

              if (coef_now > coef_main || coef_now < (1.00 / coef_main)) { //если видим скачок или спад
            ...
            break;
          case TASK_CALC_BACK_8: { //расчет текущего фона этап-8
      ...
      switch (tick_switch) { //основной блок обрабоки данных
        ...
        case TASK_UPDATE_SLEEP: //считаем время до ухода в сон
          if (mainSettings.sleep_switch) { //если сон не выключен
            ...
            if (cnt_pwr == mainSettings.time_sleep && mainSettings.sleep_switch == 2) { //если пришло время спать и сон не запрещен
              ...
              power_sleep = 1; //выставляем флаг сна
            }
...
#endif
              power_light = 1; //выставляем флаг выключенной подсветки
            }
          }
          break;
        ...
        case TASK_UPDATE_POWER: { //управление энергосбережением
            if (!sleep_disable) { //если сон разрешен
              ...
              if ((mainSettings.sleep_switch == 2) && power_manager) { //иначе если сон разрешен
                if (power_sleep && (power_manager == 2)) { //режим глубокого энергосбережения
                  sleep_status &= ~(0x01 << WAIT_PWR1);
                  sleep_status &= ~(0x01 << WAIT_PWR2);
                }
        ...
        case TASK_UPDATE_BAT: //таймер обновления состояния батареи
          if (mainTask == DEBUG_PROGRAM || mainTask == PARAMETERS_PROGRAM) {
            low_bat_massege = 1; //сбрасываем флаг обновления сообщения об разряженной акб
            ...
            if (tmr_low_bat >= LOW_BAT_TIME) {
              if (low_bat_massege) {
                ...
                low_bat_massege = 0; //устанавливаем флаг обновления сообщения об разряженной акб
              }
            }
            else if (bat_adc >= LOW_BAT_STAT) tmr_low_bat++;
          }
          break;

        case TASK_UPDATE_SCREEN: //таймер обновления экрана
          if (!power_sleep && (mainTask != SEARCH_PROGRAM)) scr = 0; //устанавливаем флаг для обновления экрана
          break;
      }
    }
...
{
  if (!sleep_status) _sleep_pwr(); //режим глубокого энергосбережения
  else if (!(sleep_status & ((0x01 << WAIT_WDT) | (0x01 << WAIT_DSP) | (0x01 << WAIT_ADC) | (0x01 << WAIT_COMP) | (0x01 << WAIT_TIM1) | (0x01 << WAIT_TIM2) | (0x01 << WAIT_PWR1)))) _save_pwr(); //режим энергосбережения
  else if (!(sleep_status & ((0x01 << WAIT_WDT) | (0x01 << WAIT_DSP) | (0x01 << WAIT_COMP)))) _wait_pwr(); //режим ожидания
}
//---------------------------------------Ожидание---------------------------------------------------------
...
{
  if (power_light) { //если выключели подсветку
#ifdef PCD8544
    _BACKL_ON(); //включаем подсветку
#endif
    power_light = 0; //сбрасываем флаг подсветки
    if (power_sleep) { //если спали
      _disable_sleep_lcd(); //выводим дисплей из сна
      _buzz_enable(); //восстанавливаем настройку щелчков
      power_sleep = 0; //сбрасываем флаг сна
      scr = 0; //разрешаем обновления экрана
    }
#ifdef SSD1306
    ...
    switch (ADMUX & 0x0F) {
      ...
      default:
        if (analogState & (0x01 << ADC_BAT)) { //замер батареи
          ...
          sleep_status |= (0x01 << WAIT_ADC); //установили флаг запрета сна
          return; //выходим
        }
#if !PUMP_FEEDBACK
  ...
  if (analogState & (0x01 << ADC_HW)) { //обратная связь
    if (TIFR0 & (0x01 << TOV0)) { //иначе если пришло время подать импульс
      if (COMP_INT_CHK || (speed_puls >= 999)) { //если накачка окончена
        ...
        sleep_status &= ~(0x01 << WAIT_COMP); //сбросили флаг запрета сна
...
#if PUMP_FEEDBACK
      drawLine(5, 10, map(timer, (START_PUMP_TIME / 50), 0, 1, 64), 0xFF); //прогресс бар накачки преобразователя
#else
      drawLine(5, 10, map(hv_adc, 0, pumpSettings.ADC_value, 1, 64), 0xFF); //прогресс бар накачки преобразователя
#endif
    }
  }
...
#if PUMP_FEEDBACK
  sleep_status |= (0x01 << WAIT_COMP); //установили флаг запрета сна
  PRR &= ~(0x01 << PRTIM0); //включаем питание таймера
  TCNT0 = 255; //сбросили счетчик
  COMP_INT_CLR; //сбрасываем флаг прерывания
#endif
}
//------------------------Статус накачки по обратной связи с АЦП------------------------------
boolean _pump_status(void) //статус накачки по обратной связи с АЦП
{
  return (analogState & ((0x01 << ADC_HW) | (0x01 << ADC_HW_DONE))); //возвращаем статус
}
//------------------------Статус накачки по обратной связи с АЦП------------------------------
boolean _pump_status_progress(void) //статус накачки по обратной связи с АЦП
{
  return (analogState & (0x01 << ADC_HW)); //возвращаем статус
}
//------------------------Статус накачки по обратной связи с АЦП------------------------------
boolean _pump_status_compleated(void) //статус накачки по обратной связи с АЦП
{
  return (analogState & (0x01 << ADC_HW_DONE)); //возвращаем статус
}
//------------------------Статус накачки по обратной связи с АЦП------------------------------
void _pump_status_compleated_set(void) //статус накачки по обратной связи с АЦП
{
  analogState |= (0x01 << ADC_HW_DONE); //устанавливаем статус
}
//------------------------Статус накачки по обратной связи с АЦП------------------------------
void _pump_status_compleated_reset(void) //статус накачки по обратной связи с АЦП
{
  ...
  switch (btn_state) { //переключаемся в зависимости от состояния клавиши
    ...
    case 1:
      if (btn_tmr > BTN_GIST_TICK) { //если таймер больше времени антидребезга
        ...
        melodyStop(); //сброс воспроизведения мелодии
        if (!power_light && !power_sleep) { //если не спим и если подсветка включена, поднимаем признак нажатия
          _disable_sleep(); //выход из сна
          switch (btn_switch) { //переключаемся в зависимости от состояния мультиопроса
...
//-------------------------------Предупреждение----------------------------------------------------------
void warn_messege(boolean set, uint8_t state, uint8_t sound) //предупреждение
{
...
void _search_update(void) //обновление данных поиска
{
  ...
  if (++score_cnt >= search_time_now) { //расчет показаний
    ...
    score_cnt = 0; //сброс
    scr = 0; //разрешаем обновление графика
  }
  ...
  while (1) {
    if (_data_update()) { //обработка данных
      ...
      if (_check_screen()) { //обновление дисплея
        ...
        if ((mainSettings.buzz_switch & 0x7F) && !mainSettings.knock_disable) drawBitmap(47, 0, buzz_alt_on_img, 7, 8); //если щелчки и зв.кнопок включен
        else if (mainSettings.buzz_switch & 0x7F) drawBitmap(47, 0, buzz_alt_img, 7, 8); //если щелчки включены и зв.кнопок выключен
