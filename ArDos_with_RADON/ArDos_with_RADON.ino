/*Arduino IDE 1.8.13
  Версия программы RADON v3.8.1 low_pwr release 20.12.21 специально для проекта ArDos
...
#define TIME_FACT_4  8 //секундные интервалы 4
...
int main(void)  //главный цикл программ
{
  ...
  for (;;) {
    ...
    switch (mainTask) {
      ...
      case 4: mainTask = logbook(); break;
      case 5: mainTask = settings(); break;
      case 6: mainTask = parameters(); break;
      case 7: mainTask = menu(); break;
...
void _data_update(void) //преобразование данных
{
  ...
  static uint8_t time_wdt; //счетчик тиков для обработки данных
  ...
  for (; tick_wdt > 0; tick_wdt--) { //если был тик, обрабатываем данные
    ...
    if (time_wdt++ > TIME_FACT_13 || (!measur && !search)) {
      switch (time_wdt) {
        ...
        case TIME_FACT_8: //рассчитываем точность
        ...
        case TIME_FACT_18: //таймер обновления батареи
...
uint8_t measur_menu(void) //режим замера
{
  ...
  while (1) {
    _data_update(); //преобразование данных
    _bat_massege(); //обновление состояния батареи
...
void _start_pump(void) //первая накачка
{
  ...
  for (hv_adc = Read_HV(); hv_adc < pumpSettings.ADC_value; hv_adc = Read_HV()) { //значение АЦП при котором на выходе 400В
    ...
    for (uint8_t cnt = pumpSettings.puls; cnt > 0; cnt--) asm("nop"); //ждем
    CONV_OFF; //пин накачки выкл

    if (++timer >= START_PUMP_TIME) break; //если время вышло, останавливаем накачку
  ...
  for (uint8_t i = 0; i < height; i++) { //отрисовываем 3 строки
    ...
    if (val >= 8) { //если уровень больше 8
      ...
      while (val) { //пока есть уровень
        ...
        bits |= 0x01; //устанавливаем бит в буфер
...
//-------------------------------------------Разрешить сон--------------------------------------------------
void sleep_ret(void) //разрешить сон
{
  sleep_disable = 0; //разрешаем сон
  if (cnt_pwr > mainSettings.time_bright) cnt_pwr = mainSettings.time_bright + 1; //откатываем состояние таймера сна
}
//-----------------------------------------Сброс текущей дозы-----------------------------------------------
        ...
        switch (alarm_switch) { //режим тревоги
          case 0:
            switch (scr_mode) { //основные экраны
              ...
              case 1: data_reset(dose_mode); sleep_ret(); break; //сбрасываем дозу и время
            }
            break;
