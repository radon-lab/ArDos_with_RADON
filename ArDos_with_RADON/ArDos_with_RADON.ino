...
#define SOUND_START(freq) PRR &= ~(0x01 << PRTIM1); ICR1 = freq; OCR1A = ((freq / 2) / 10 * mainSettings.volume); TCNT1 = 0x00; TIMSK1 = (0x01 << OCIE1A) | (0x01 << TOIE1)
#define SOUND_STOP        TIMSK1 = 0x00; PRR |= (0x01 << PRTIM1)
...
#define BUZZ_START        cnt_puls = BUZZ_CYCLE; PRR &= ~(0x01 << PRTIM1); ICR1 = BUZZ_TIMER; OCR1A = buzz_vol; TCNT1 = 0x00; TIMSK1 = (0x01 << OCIE1A) | (0x01 << TOIE1)
#define BUZZ_VOL_SET(vol) buzz_vol = (BUZZ_TIMER / 2) / 10 * vol;

uint16_t buzz_vol; //громкость щелчков

//вспышки старт/стоп
#define _RAD_FLASH_ON  {PRR &= ~(0x01 << PRTIM0); TIMSK0 = (0x01 << TOIE0); RAD_FLASH_ON;}
#define _RAD_FLASH_OFF {TIMSK0 = 0x00; PRR |= (0x01 << PRTIM0); RAD_FLASH_OFF;}

//перевод напряжения в значение ацп
#define LOW_BAT_POWER (uint8_t)((DEFAULT_REFERENCE * 256.0) / LOW_BAT_POWER_V)
#define LOW_BAT_STAT (uint8_t)((DEFAULT_REFERENCE * 256.0) / LOW_BAT_STAT_V)
#define MIN_BAT (uint8_t)((DEFAULT_REFERENCE * 256.0) / MIN_BAT_V)
#define MAX_BAT (uint8_t)((DEFAULT_REFERENCE * 256.0) / MAX_BAT_V)
...
void _init_timers(void) //инициализация таймеров
{
  ...
  TCCR2B = (0x01 << CS22) | (0x01 << CS20); //пределитель 256
...
void _adc_enable(void) //включение ADC
{
  ...
  ADCSRA = (0x01 << ADEN) | (0x01 << ADPS2) | (0x01 << ADPS1) | (0x01 << ADPS0); //настройка АЦП
}
//----------------------------------Выключение ADC---------------------------------------------------
...
uint8_t _read_HV(void) //чтение напряжения преобразователя
{
  ...
  return hv_adc; //возвращаем результат опроса АЦП
}
//-----------------------------------Первая накачка-------------------------------------------
void _start_pump(void) //первая накачка
{
  uint16_t timer = START_PUMP_IMP; //таймер авто-выхода
  print(LOAD, CENTER, 32); //загрузка...

  while (hv_adc < pumpSettings.ADC_value) { //значение АЦП при котором на выходе 400В
    _pump_converter(); //накачка по обратной связи с АЦП
    ...
    tick_buff = 0; //очищаем буфер тиков
    if (!timer--) break; //если время вышло то останавливаем накачку
  }
}
//----------------------------Накачка по обратной связи с АЦП---------------------------------
void _pump_converter(void) //накачка по обратной связи с АЦП
{
  uint8_t timer = CYCLE_OVERFLOW; //счетчик циклов переполнения

  while (_read_HV() < pumpSettings.ADC_value) { //значение АЦП при котором на выходе 400В
    _pump_puls(pumpSettings.puls); //ипульс на преобразователь

    speed_pump++; //считаем скорость накачки
    if (!timer--) break; //защита от зацикливания
  }
}
...
uint8_t _read_VCC(void)  //чтение напряжения батареи
{
  ADMUX = (0x01 << REFS0) | (0x01 << ADLAR) | (0x01 << MUX3) | (0x01 << MUX2) | (0x01 << MUX1); //выбор внешнего опорного+BG
...
{
  bat_adc = _read_VCC(); //состояние батареи
