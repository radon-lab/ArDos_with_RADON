...
{
  if (++tick_buff == 255) RESET_SYSTEM; //если буфер переполнен то перезагружаемся
}
...
void _measur_massege(void) //окончание замера
{
  if (next_measur && !alarm_measur) { //если поднят флаг следующего замера и оповещение окончания замера разешено
    ...
    _buzz_enable(); //считываем настроку щелчков
  }
}
//-------------------------------Режим замера----------------------------------------------------------
...
void alarm_messege(boolean set, uint8_t sound, const char *mode) //тревога
{
  ...
  while (1) {
    if (_data_update()) { //обработка данных
      ...
      {
        _vibro_off(); //выключаем вибрацию
        _buzz_enable(); //восстанавливаем настроку щелчков
        ...
        case DOWN_KEY_PRESS: //сброс
          switch (alarm_switch) { //режим тревоги
            case 0:
              ...
              break;
            case 3: warn_back_wait = 1; alarm_switch = 0; _vibro_off(); _buzz_enable(); break; //сбросить тревогу фон
            case 4: warn_dose_wait = rad_dose; alarm_switch = 0; _vibro_off(); _buzz_enable(); break; //сбросить тревогу доза
          }
          break;
